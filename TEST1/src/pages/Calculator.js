...
const Calculator = () => {
	...
	useEffect(() => {
		...
		if (
			is5Clicked === true &&
			otts.map(row => row.ott_name).includes('Apple TV Plus') === true
		)
			setSelectedOTT('Apple TV Plus');
		if (
			is6Clicked === true &&
			otts.map(row => row.ott_name).includes('Amazon Prime Video') === true
		)
			setSelectedOTT('Amazon Prime Video');
		printNumber();
	});
	...
	const printNumber = () => {
		...
		for (let i = 0; i < otts.length; i++)
			if (selectedOTT === otts[i].ott_name) {
				number = otts[i].won_per_min.toFixed(0);
			}
		return number;
	};
							...
							<div style={{ padding: '5vh 4vw 5vh 4vw' }}>
								...
								<div style={{ display: 'flex' }}>
									<DisableWrapper>
										...
										<DisableRect
											...
											onClick={setStyle5}
											style={{
												display: styleBoolean('Apple TV Plus')
													? 'none'
													: 'block',
											}}
										/>
										<DisableRect
											...
											onClick={setStyle6}
											style={{
												display: styleBoolean('Amazon Prime Video')
													? 'none'
													: 'block',
											}}
										/>
								...
								<OTTWrapper>
									...
									<AppleTVRect
										className='ottrect5'
										style={{
											border: styleBoolean('Apple TV Plus')
												? '1px solid #FFFFFF'
												: '0px',
											boxShadow: styleBoolean('Apple TV Plus')
												? '0px 0px 1.8vw 0.5vw #FFF'
												: '0px 0px 0px 0px #FFF',
											borderRadius: '1.7vw',
										}}
									...
									<PrimeVideoRect
										className='ottrect6'
										style={{
											border: styleBoolean('Amazon Prime Video')
												? '1px solid #FFFFFF'
												: '0px',
											boxShadow: styleBoolean('Amazon Prime Video')
												? '0px 0px 1.8vw 0.5vw #FFF'
												: '0px 0px 0px 0px #FFF',
											borderRadius: '1.7vw',
										}}
